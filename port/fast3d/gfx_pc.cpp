...
static std::map<ColorCombinerKey, struct ColorCombiner>::iterator
...
static Vertex buf_vbo[MAX_BUFFERED * 3]; // 3 verts × 9 floats/vert
...
gfx_lookup_or_create_shader_program(uint64_t shader_id0, uint32_t shader_id1) {
...
static void gfx_generate_cc(struct ColorCombiner *comb,
  ...
  for (int i = 0; i < 2 && (i == 0 || is_2cyc); i++) {
    ...
    if (i == 1) {
      if (rgb_a != G_CCMUX_COMBINED && rgb_b != G_CCMUX_COMBINED &&
          rgb_c != G_CCMUX_COMBINED && rgb_d != G_CCMUX_COMBINED) {
        // First cycle RGB not used, so clear it away
        c[0][0][0] = c[0][0][1] = c[0][0][2] = c[0][0][3] = G_CCMUX_0;
      }
      if (rgb_c != G_CCMUX_COMBINED_ALPHA && alpha_a != G_ACMUX_COMBINED &&
          alpha_b != G_ACMUX_COMBINED && alpha_d != G_ACMUX_COMBINED) {
  ...
  memcpy(comb->shader_input_mapping, shader_input_mapping,
         sizeof(shader_input_mapping));
}

// Resize buffer if NPOT. Returns pointer to new buffer and sets POT size.
static const uint8_t *fix_npot_texture(const uint8_t *rgba32_buf,
                                       uint32_t width, uint32_t height,
                                       uint32_t &out_pot_w, uint32_t &out_pot_h,
                                       std::vector<uint8_t> &temp) {
  out_pot_w = 1;
  out_pot_h = 1;
  while (out_pot_w < width)
    out_pot_w <<= 1;
  while (out_pot_h < height)
    out_pot_h <<= 1;
...
// ------------------------------------------------------------------
static const uint8_t *mirror_texture_rgba32(const uint8_t *src, uint32_t src_w,
                                            uint32_t src_h, bool mirror_s,
                                            bool mirror_t,
...
static struct ColorCombiner *
gfx_lookup_or_create_color_combiner(const ColorCombinerKey &key) {
  if (prev_combiner != color_combiner_pool.end() &&
      prev_combiner->first == key) {
  ...
  gfx_flush();
  prev_combiner =
      color_combiner_pool.insert(std::make_pair(key, ColorCombiner())).first;
...
static bool gfx_texture_cache_lookup(int i, const TextureCacheKey &key) {
  ...
  if (it != gfx_texture_cache.map.end()) {
    gfx_rapi->select_texture(i, it->second.texture_id,
                             it->second.linear_filter);
    *n = &*it;
    gfx_texture_cache.lru.splice(gfx_texture_cache.lru.end(),
                                 gfx_texture_cache.lru,
  ...
  it = gfx_texture_cache.map.insert(std::make_pair(key, TextureCacheValue()))
           .first;
  TextureCacheNode *node = &*it;
  node->second.texture_id = texture_id;
  node->second.lru_location =
      gfx_texture_cache.lru.insert(gfx_texture_cache.lru.end(), {it});
...
void gfx_texture_cache_delete(const uint8_t *orig_addr) {
  ...
  for (int i = 0; i < 2; ++i) {
    if (rendering_state.textures[i] &&
        rendering_state.textures[i]->first.texture_addr == orig_addr) {
  ...
  while (gfx_texture_cache.map.bucket_count() > 0) {
    TextureCacheKey key = {
        orig_addr, {0}, 0, 0}; // bucket index only depends on the address
    size_t bucket = gfx_texture_cache.map.bucket(key);
    bool again = false;
    for (auto it = gfx_texture_cache.map.begin(bucket);
         it != gfx_texture_cache.map.end(bucket); ++it) {
...
static void import_texture_rgba16(int tile, const LoadedTexture &loaded_texture,
  ...
  // TODO: this trips in some places with a garbage size in
  // full_image_line_size_bytes probably wherever framebuffer effects are used
  ...
  //     const uint8_t* src = fix_npot_texture((const
  //     uint8_t*)tex_upload_buffer, width, height, pot_w, pot_h, temp_pot_buf);
  //     // ------------------------------------------------------------------
  ...
  while (pot_w < (int)width)
    pot_w <<= 1;
  while (pot_h < (int)height)
    pot_h <<= 1;
  if (rendering_state.textures[0]) {
    rendering_state.textures[0]->second.pot_w = pot_w;
    rendering_state.textures[0]->second.pot_h = pot_h;
...
static void import_texture_rgba32(int tile, const LoadedTexture &loaded_texture,
                                  bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
  ...
  const uint32_t height =
      (size_bytes / 2) / rdp.texture_tile[tile].line_size_bytes;

  //     std::vector<uint8_t> temp_pot_buf;
  //     uint32_t pot_w, pot_h;
  //     const uint8_t* src8 = fix_npot_texture((const
  //     uint8_t*)tex_upload_buffer, width, height, pot_w, pot_h, temp_pot_buf);
  //     // ------------------------------------------------------------------
  //     // Software fallback for MIRRORED_REPEAT on GLES 1.1 / PSP GU
  //     // ------------------------------------------------------------------
...
static void import_texture_ia4(int tile, const LoadedTexture &loaded_texture,
                               bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
...
static void import_texture_ia8(int tile, const LoadedTexture &loaded_texture,
                               bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
...
static void import_texture_ia16(int tile, const LoadedTexture &loaded_texture,
                                bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
...
static void import_texture_i4(int tile, const LoadedTexture &loaded_texture,
                              bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
...
static void import_texture_i8(int tile, const LoadedTexture &loaded_texture,
                              bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
...
static inline void palette_to_rgba32(const uint16_t palentry,
                                     uint8_t *rgba32_buf) {
...
static void import_texture_ci4(int tile, const LoadedTexture &loaded_texture,
                               bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
  ...
  const uint16_t *palette =
      (const uint16_t *)(rdp.palette +
                         pal_idx * 16); // 16 pixel entries, 16 bits each
...
static void import_texture_ci8(int tile, const LoadedTexture &loaded_texture,
                               bool importReplacement) {
  const RawTexMetadata *metadata = &loaded_texture.raw_tex_metadata;
  const uint8_t *addr = loaded_texture.addr;
  const uint32_t size_bytes = loaded_texture.size_bytes;
  ...
  for (uint32_t i = 0, j = 0; i < size_bytes;
       j += full_image_line_size_bytes - line_size_bytes) {
...
static void import_texture(int i, int tile, bool importReplacement) {
  ...
  if ((rdp.tex_lod && tile >= rdp.first_tile_index + rdp.tex_detail) ||
      !loaded_texture.addr) {
    // set up miplevel 0; also acts as a catch-all for when .addr is NULL
    // because my texture loader sucks
    ...
    loaded_texture.full_size_bytes = loaded_texture.full_image_line_size_bytes *
                                     rdp.texture_tile[tile].height;
    loaded_texture.size_bytes =
        loaded_texture.line_size_bytes * rdp.texture_tile[tile].height;
    if (siz == G_IM_SIZ_32b) {
  ...
  TextureCacheKey key;
  if (fmt == G_IM_FMT_CI) {
    key = TextureCacheKey{orig_addr,
                          {rdp.palette_addrs[0], rdp.palette_addrs[1]},
                          fmt,
                          siz,
                          palette_index,
                          tileinfo.uls,
    ...
    key = TextureCacheKey{
        orig_addr,      {nullptr, nullptr}, fmt,          siz,
        palette_index,  tileinfo.uls,       tileinfo.ult, tileinfo.lrs,
        tileinfo.lrt,   tileinfo.cms,       tileinfo.cmt, tileinfo.width,
        tileinfo.height};
  }
...
static void gfx_transposed_matrix_mul(float res[3], const float a[3],
                                      const float b[4][4]) {
...
static void calculate_normal_dir(const Light_t *light, float coeffs[3]) {
  const float light_dir[3] = {light->dir[0] / 127.f, light->dir[1] / 127.f,
                              light->dir[2] / 127.f};
...
static void calculate_normal_dir(const struct NormalColor *vcn,
                                 float coeffs[3]) {
...
static void gfx_matrix_mul(float res[4][4], const float a[4][4],
                           const float b[4][4]) {
  float tmp[4][4];
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
...
static void gfx_sp_matrix(uint8_t parameters, const int32_t *addr) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j += 2) {
      ...
      matrix[i][j + 1] =
          (int32_t)((int_part << 16) | (frac_part & 0xffff)) / 65536.0f;
    }
  }
#else
...
static float gfx_adjust_x_for_aspect_ratio(float x, float w = 1.f) {
  if (fbActive) {
    ...
    return (rsp.aspect_ofs * w + x) * rsp.aspect_scale /
           gfx_current_dimensions.aspect_ratio;
  }
}

static void gfx_adjust_width_height_for_scale(uint32_t &width,
                                              uint32_t &height) {
...
static void gfx_sp_vertex(size_t n_vertices, size_t dest_index,
  ...
  for (size_t i = 0; i < n_vertices; i++, dest_index++) {
    ...
    float x = v->v[0] * rsp.MP_matrix[0][0] + v->v[1] * rsp.MP_matrix[1][0] +
              v->v[2] * rsp.MP_matrix[2][0] + rsp.MP_matrix[3][0];
    float y = v->v[0] * rsp.MP_matrix[0][1] + v->v[1] * rsp.MP_matrix[1][1] +
              v->v[2] * rsp.MP_matrix[2][1] + rsp.MP_matrix[3][1];
    float z = v->v[0] * rsp.MP_matrix[0][2] + v->v[1] * rsp.MP_matrix[1][2] +
...
//     w = 1.0f;
// #endif
    // If aspect ration changed adjust x using perspective
    ...
    // Change lighting based on where the player is looking
    if (rsp.geometry_mode & G_LIGHTING) {
      if (rsp.lights_changed) {
        for (int i = 0; i < rsp.current_num_lights - 1; i++) {
          calculate_normal_dir(&rsp.current_lights[i],
                               rsp.current_lights_coeffs[i]);
        }
...
//       d->clip_rej |= 32; // CLIP_FAR
    ...
    d->color.a =
        vcn->a; // can be required for SHADE_ALPHA even if fog is enabled
  }
}

static void gfx_sp_modify_vertex(uint16_t vtx_idx, uint8_t where,
                                 uint32_t val) {
...
static void gfx_sp_tri1(uint8_t v1_idx, uint8_t v2_idx, uint8_t v3_idx, bool is_rect) {
    // Get vertices for triangle
    struct LoadedVertex *v1 = &rsp.loaded_vertices[v1_idx];
    struct LoadedVertex *v2 = &rsp.loaded_vertices[v2_idx];
    struct LoadedVertex *v3 = &rsp.loaded_vertices[v3_idx];
...
//     }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // This was a loop before, TODO: figure out a way to layer textures without shaders
        auto t = 0;
...
static inline void gfx_update_aspect_mode(void) {
  ...
  rsp.aspect_scale = rsp.aspect_mode
                         ? gfx_current_native_aspect
                         : gfx_current_window_dimensions.aspect_ratio;

  if (side == G_ASPECT_LEFT_EXT) {
    rsp.aspect_ofs =
        1.f - gfx_current_dimensions.aspect_ratio / gfx_current_native_aspect;
  } else if (side == G_ASPECT_RIGHT_EXT) {
    rsp.aspect_ofs =
        gfx_current_dimensions.aspect_ratio / gfx_current_native_aspect - 1.f;
  } else {
    rsp.aspect_ofs = 0.f;
  }
...
static void gfx_adjust_viewport_or_scissor(XYWidthHeight *area,
  ...
  if (preserve_aspect) {
    ...
    const float ratio =
        gfx_current_native_aspect / gfx_current_dimensions.aspect_ratio;
  ...
  if (!game_renders_to_framebuffer ||
      (gfx_msaa_level > 1 &&
       gfx_current_dimensions.width == gfx_current_game_window_viewport.width &&
...
static void gfx_sp_movemem(uint8_t index, uint8_t offset, const void *data) {
  ...
  case G_MV_LOOKATX:
    ...
    rsp.lookat_enabled =
        (index == 0) || (rsp.lookat[1].dir[0] || rsp.lookat[1].dir[1]);
...
static void gfx_sp_texture(uint16_t sc, uint16_t tc, uint8_t level,
                           uint8_t tile, uint8_t on) {
...
static void gfx_dp_set_scissor(uint32_t mode, uint32_t ulx, uint32_t uly,
...
static void gfx_dp_set_texture_image(uint32_t format, uint32_t size,
                                     uint32_t width, uint32_t tex_flags,
                                     const void *addr) {
...
static void gfx_dp_set_tile(uint8_t fmt, uint32_t siz, uint32_t line,
                            uint32_t tmem, uint8_t tile, uint32_t palette,
                            uint32_t cmt, uint32_t maskt, uint32_t shiftt,
  ...
  rdp.texture_tile[tile].palette =
      palette; // palette should set upper 4 bits of color index in 4b mode
...
static void gfx_dp_set_tile_size(uint8_t tile, uint16_t uls, uint16_t ult,
...
static void gfx_dp_load_tlut(uint8_t tile, uint32_t uls, uint32_t ult,
  ...
  const uint16_t *base =
      (const uint16_t *)rdp.texture_to_load.addr + pitch * ult + uls;
...
static void gfx_dp_load_block(uint8_t tile, uint32_t uls, uint32_t ult,
...
static void gfx_dp_load_tile(uint8_t tile, uint32_t uls, uint32_t ult,
                             uint32_t lrs, uint32_t lrt) {
  SUPPORT_CHECK(tile == G_TX_LOADTILE);
  ...
  uint32_t full_image_line_size_bytes =
      full_image_width << rdp.texture_to_load.siz >> 1;

  uint32_t orig_size_bytes = tile_line_size_bytes * tile_height;
  uint32_t size_bytes = orig_size_bytes;
  uint32_t start_offset_bytes =
      full_image_line_size_bytes * offset_y + offset_x_in_bytes;
  ...
  if (h_byte_scale != 1 || v_pixel_scale != 1) {
    start_offset_bytes =
        h_byte_scale * (v_pixel_scale * offset_y * full_image_line_size_bytes +
                        offset_x_in_bytes);
...
static void gfx_dp_set_combine_mode(uint32_t rgb, uint32_t alpha,
                                    uint32_t rgb_cyc2, uint32_t alpha_cyc2) {
  rdp.combine_mode = rgb | (alpha << 16) | ((uint64_t)rgb_cyc2 << 28) |
                     ((uint64_t)alpha_cyc2 << 44);
}

static inline uint32_t color_comb(uint32_t a, uint32_t b, uint32_t c,
                                  uint32_t d) {
  return (a & 0xf) | ((b & 0xf) << 4) | ((c & 0x1f) << 8) | ((d & 7) << 13);
}

static inline uint32_t alpha_comb(uint32_t a, uint32_t b, uint32_t c,
                                  uint32_t d) {
  return (a & 7) | ((b & 7) << 3) | ((c & 7) << 6) | ((d & 7) << 9);
}

static void gfx_dp_set_grayscale_color(uint8_t r, uint8_t g, uint8_t b,
                                       uint8_t a) {
...
static void gfx_dp_set_prim_color(uint8_t m, uint8_t l, uint8_t r, uint8_t g,
                                  uint8_t b, uint8_t a) {
...
static void gfx_draw_rectangle(int32_t ulx, int32_t uly, int32_t lrx,
                               int32_t lry) {
  ...
  struct XYWidthHeight default_viewport = {0, (int16_t)SCREEN_HEIGHT,
...
static void gfx_dp_texture_rectangle(int32_t ulx, int32_t uly, int32_t lrx,
                                     int32_t lry, uint8_t tile, int16_t uls,
                                     int16_t ult, int16_t dsdx, int16_t dtdy,
                                     bool flip) {
  uint64_t saved_combine_mode = rdp.combine_mode;
  if ((rdp.other_mode_h & (3U << G_MDSFT_CYCLETYPE)) == G_CYC_COPY) {
    // Per RDP Command Summary Set Tile's shift s and this dsdx should be set to
    // 4 texels Divide by 4 to get 1 instead
    dsdx >>= 2;

    // Color combiner is turned off in copy mode
    gfx_dp_set_combine_mode(color_comb(0, 0, 0, G_CCMUX_TEXEL0),
                            alpha_comb(0, 0, 0, G_ACMUX_TEXEL0), 0, 0);
...
static void gfx_dp_image_rectangle(int32_t tile, int32_t w, int32_t h,
                                   int32_t ulx, int32_t uly, int16_t uls,
                                   int16_t ult, int32_t lrx, int32_t lry,
                                   int16_t lrs, int16_t lrt) {
  uint64_t saved_combine_mode = rdp.combine_mode;
  ...
  loadtex.full_image_line_size_bytes = loadtex.line_size_bytes =
      rdp.texture_tile[tile].line_size_bytes;
  loadtex.size_bytes = loadtex.orig_size_bytes = loadtex.full_size_bytes =
      loadtex.line_size_bytes * h;
...
static void gfx_dp_fill_rectangle(int32_t ulx, int32_t uly, int32_t lrx,
                                  int32_t lry) {
  ...
  // OTRTODO: This is a bit of a hack for widescreen screen fades, but it'll
  // work for now...
  ...
  if (mode == G_CYC_FILL) {
    gfx_dp_set_combine_mode(color_comb(0, 0, 0, G_CCMUX_SHADE),
                            alpha_comb(0, 0, 0, G_ACMUX_SHADE), 0, 0);
  }
...
static void gfx_dp_set_color_image(uint32_t format, uint32_t size,
                                   uint32_t width, void *address) {
  rdp.color_image_address = address;
}

static void gfx_sp_set_other_mode(uint32_t shift, uint32_t num_bits,
                                  uint64_t mode) {
  ...
  rdp.tex_detail =
      (rdp.other_mode_h & (2U << G_MDSFT_TEXTDETAIL)) == G_TD_DETAIL;
}

static void gfx_sp_set_vertex_colors(uint32_t count,
                                     const struct NormalColor *vcn) {
  // common sense dictates that we should copy the colors as the command is
  // supposed to do, but it actually doesn't seem to matter SUPPORT_CHECK(count
  // <= sizeof(rsp.vertex_colors) / sizeof(rsp.vertex_colors[0])); for (uint32_t
  // i = 0; i < count; ++i) {
...
static void gfx_run_dl(Gfx *cmd) {
  ...
  for (;;) {
    ...
    case (uint8_t)G_TEXTURE:
      ...
      break;
    case G_VTX:
      gfx_sp_vertex(C0(0, 16) / sizeof(Vtx), C0(16, 4),
    ...
    case (uint8_t)G_SETOTHERMODE_H:
      gfx_sp_set_other_mode(C0(8, 8) + 32, C0(0, 8),
                            (uint64_t)cmd->words.w1 << 32);
      break;
    case G_COL:
      gfx_sp_set_vertex_colors(C0(0, 16) / 4,
                               (NormalColor *)seg_addr(cmd->words.w1));
      break;

    // RDP Commands:
    case G_SETTIMG: {
      gfx_dp_set_texture_image(C0(21, 3), C0(19, 2), C0(0, 10), 0,
    ...
    case G_SETGRAYSCALE_EXT:
      ...
      break;
    case G_LOADBLOCK:
      gfx_dp_load_block(C1(24, 3), C0(12, 12), C0(0, 12), C1(12, 12),
                        C1(0, 12));
      break;
    case G_LOADTILE:
      ...
      break;
    case G_SETTILE:
      gfx_dp_set_tile(C0(21, 3), C0(19, 2), C0(9, 9), C0(0, 9), C1(24, 3),
                      C1(20, 4), C1(18, 2), C1(14, 4), C1(10, 4), C1(8, 2),
                      C1(4, 4), C1(0, 4));
      break;
    case G_SETTILESIZE:
      gfx_dp_set_tile_size(C1(24, 3), C0(12, 12), C0(0, 12), C1(12, 12),
                           C1(0, 12));
      break;
    case G_LOADTLUT:
    ...
    case G_SETENVCOLOR:
      ...
      break;
    case G_SETPRIMCOLOR:
      gfx_dp_set_prim_color(C0(8, 8), C0(0, 8), C1(24, 8), C1(16, 8), C1(8, 8),
                            C1(0, 8));
      break;
    case G_SETFOGCOLOR:
    ...
    case G_TEXRECTFLIP: {
      ...
      dsdx = C1(16, 16);
      dtdy = C1(0, 16);
      gfx_dp_texture_rectangle(ulx, uly, lrx, lry, tile, uls, ult, dsdx, dtdy,
    ...
    case G_SETSCISSOR:
      gfx_dp_set_scissor(C1(24, 2), C0(12, 12), C0(0, 12), C1(12, 12),
                         C1(0, 12));
      break;
    case G_SETZIMG:
      ...
      break;
    case G_SETCIMG:
      gfx_dp_set_color_image(C0(21, 3), C0(19, 2), C0(0, 11),
                             seg_addr(cmd->words.w1));
      break;
    case G_SETFB_EXT:
      gfx_flush();
      ...
      break;
    case G_COPYFB_EXT:
      gfx_copy_framebuffer(C0(11, 11), C0(0, 11), (int16_t)C1(16, 16),
                           (int16_t)C1(0, 16), C0(22, 1));
      break;
    ...
    case (uint8_t)G_RDPHALF_CONT:
      // on N64 skyRender uses these to render some types of skies and skybox
      // water by issuing low-level ucode commands G_TRI_FILL and
      // G_TRI_SHADE_TXTR the port renders the sky in a different manner
      break;
    case G_RDPFLUSH_EXT:
      gfx_flush();
      break;
    ...
    default:
      sysFatalError("Unknown GBI opcode 0x%02x at %p.\nw0 %08x\nw1 %08x",
                    opcode, cmd, cmd->words.w0, cmd->words.w1);
      break;
    }
    ++cmd;
  }
...
extern "C" void gfx_get_dimensions(uint32_t *width, uint32_t *height,
...
extern "C" void gfx_init(const GfxInitSettings *settings) {
  ...
  gfx_rapi->init();
  gfx_rapi->update_framebuffer_parameters(0, settings->window_settings.width,
                                          settings->window_settings.height, 1,
  ...
  if (gfx_msaa_level > 1 && !gfx_framebuffers_enabled) {
    sysLogPrintf(
        LOG_WARNING,
        "F3D: MSAA set to %d, but framebuffers are not available; disabling",
        gfx_msaa_level);
    gfx_msaa_level = 1;
  }
...
extern "C" void gfx_destroy(void) {
  // TODO: should also destroy rapi and wapi, and any other resources acquired
  // in fast3d

  // Texture cache and loaded textures store references to Resources which need
  // to be unreferenced.
  gfx_texture_cache_clear();
}
...
extern "C" void gfx_start_frame(void) {
  ...
  gfx_wapi->get_dimensions(&gfx_current_window_dimensions.width,
                           &gfx_current_window_dimensions.height,
                           &gfx_current_window_position_x,
                           &gfx_current_window_position_y);
  ...
  gfx_current_window_dimensions.aspect_ratio =
      (float)gfx_current_window_dimensions.width /
      gfx_current_window_dimensions.height;
  ...
  if (gfx_current_dimensions.height != gfx_prev_dimensions.height) {
    for (auto &fb : framebuffers) {
      ...
      if (fb.second.autoresize) {
        ...
        if (width != fb.second.applied_width ||
            height != fb.second.applied_height) {
          gfx_rapi->update_framebuffer_parameters(fb.first, width, height, 1,
                                                  true, true, true, true);
  ...
  if (gfx_framebuffers_enabled && (different_size || gfx_msaa_level > 1)) {
    ...
    if (different_size) {
      gfx_rapi->update_framebuffer_parameters(
          game_framebuffer, gfx_current_dimensions.width,
          gfx_current_dimensions.height, gfx_msaa_level, true, true, true,
          true);
    } else {
      // MSAA framebuffer needs to be resolved to an equally sized target when
      // complete, which must therefore match the window size
      gfx_rapi->update_framebuffer_parameters(
          game_framebuffer, gfx_current_window_dimensions.width,
          gfx_current_window_dimensions.height, gfx_msaa_level, false, true,
          true, true);
    }
    if (gfx_msaa_level > 1 && different_size) {
      gfx_rapi->update_framebuffer_parameters(
          game_framebuffer_msaa_resolved, gfx_current_dimensions.width,
...
extern "C" void gfx_run(Gfx *commands) {
  ...
  gfx_rapi->update_framebuffer_parameters(
      0, gfx_current_window_dimensions.width,
      gfx_current_window_dimensions.height, 1, false, true, true,
      !game_renders_to_framebuffer);
  gfx_rapi->start_frame();
  gfx_rapi->start_draw_to_framebuffer(
      game_renders_to_framebuffer ? game_framebuffer : 0,
  ...
  if (game_renders_to_framebuffer) {
    ...
    if (gfx_msaa_level > 1) {
      bool different_size = gfx_current_dimensions.width !=
                                gfx_current_game_window_viewport.width ||
                            gfx_current_dimensions.height !=
                                gfx_current_game_window_viewport.height;

      if (different_size) {
        gfx_rapi->resolve_msaa_color_buffer(game_framebuffer_msaa_resolved,
                                            game_framebuffer);
        gfxFramebuffer = (uintptr_t)gfx_rapi->get_framebuffer_texture_id(
            game_framebuffer_msaa_resolved);
      } else {
        gfx_rapi->resolve_msaa_color_buffer(0, game_framebuffer);
      }
    } else {
      gfxFramebuffer =
          (uintptr_t)gfx_rapi->get_framebuffer_texture_id(game_framebuffer);
    }
  }
...
extern "C" void gfx_set_target_fps(int fps) { gfx_wapi->set_target_fps(fps); }
...
extern "C" int gfx_create_framebuffer(uint32_t width, uint32_t height,
                                      int upscale, int autoresize) {
  int fb = gfx_rapi->create_framebuffer();
  gfx_resize_framebuffer(fb, width, height, upscale, autoresize);
  return fb;
}

extern "C" void gfx_resize_framebuffer(int fb, uint32_t width, uint32_t height,
                                       int upscale, int autoresize) {
  uint32_t orig_width, orig_height;
  ...
  if (width && height) {
    ...
    gfx_rapi->update_framebuffer_parameters(fb, width, height, 1, true, true,
                                            true, true);
  } else {
  ...
  framebuffers[fb] = {orig_width, orig_height,   width,
                      height,     (bool)upscale, (bool)autoresize};
}
...
extern "C" void gfx_copy_framebuffer(int fb_dst, int fb_src, int left, int top,
  ...
  if (is_main_fb) {
    if (left > 0 && top > 0) {
      ...
      left = left * gfx_current_dimensions.width /
             gfx_current_native_viewport.width;
      top = top * gfx_current_dimensions.height /
            gfx_current_native_viewport.height;
      // flip Y
  ...
  gfx_rapi->copy_framebuffer(fb_dst, fb_src, left, top, is_main_fb,
                             (bool)use_back);
}

extern "C" void gfx_reset_framebuffer(void) {
  gfx_rapi->start_draw_to_framebuffer(0, (float)gfx_current_dimensions.height /
                                             SCREEN_HEIGHT);
